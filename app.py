...
def process_table_aggregation(df_input):
    if df_input.empty:
        return df_input

    # Chuẩn hóa tên cột để xử lý thống nhất
    col_mapping = {}
    for col in df_input.columns:
        col_clean = clean_str(col).lower()
        if 'trường' in col_clean or 'truong' in col_clean:
            col_mapping[col] = 'Tên trường'
        elif 'mã' in col_clean or 'ma' in col_clean:
            col_mapping[col] = 'Mã ngành'
        elif 'ngành' in col_clean or 'nganh' in col_clean:
            col_mapping[col] = 'Tên ngành'
        elif 'điểm' in col_clean or 'diem' in col_clean:
            col_mapping[col] = 'Điểm chuẩn'
        elif 'phương thức' in col_clean or 'phuong thuc' in col_clean or 'ptxt' in col_clean:
            col_mapping[col] = 'Phương thức xét tuyển'

    df_renamed = df_input.rename(columns=col_mapping)
    required_cols = ['Tên trường', 'Tên ngành', 'Mã ngành', 'Điểm chuẩn', 'Phương thức xét tuyển']

    # Nếu thiếu cột chính thì trả về DataFrame ban đầu
    if not all(col in df_renamed.columns for col in required_cols):
        return df_input

    df_work = df_renamed[required_cols].copy()
    
    # Làm sạch dữ liệu trước khi so sánh gom nhóm
    df_work['Tên trường'] = df_work['Tên trường'].apply(clean_str)
    df_work['Tên ngành'] = df_work['Tên ngành'].apply(clean_str)
    df_work['Mã ngành'] = df_work['Mã ngành'].apply(clean_str)
    df_work['Điểm chuẩn'] = df_work['Điểm chuẩn'].apply(clean_score)

    key_cols = ['Tên trường', 'Tên ngành', 'Mã ngành', 'Điểm chuẩn']
    
    # Gom nhóm dữ liệu trùng 4 trường, nối tất cả Phương thức xét tuyển cách nhau bằng dấu phẩy
    grouped = df_work.groupby(key_cols, as_index=False, sort=False).agg({
...
with st.container(border=True):
    ...
    if df_dh is not None and not df_dh.empty:
        col_attr1, col_attr2 = st.columns([3, 1], vertical_alignment="bottom")
        with col_attr1:
            attr_options = [
            ...
            selected_attr = st.selectbox(t["analysis_attr_label"], attr_options, key="select_analysis_attr")
        with col_attr2:
            btn_analyze = st.button(t["analysis_btn"], use_container_width=True)
            
        if btn_analyze:
            with st.spinner(t["analysis_searching"]):
                search_term = selected_attr.lower().strip()
                match_mask = df_dh.astype(str).apply(lambda row: row.str.lower().str.contains(search_term, regex=False)).any(axis=1)
                filtered_df = df_dh[match_mask].copy()
                
                if not filtered_df.empty:
                    st.markdown(f"#### {t['analysis_table_title'].format(selected_attr)}")
                    aggregated_df = process_table_aggregation(filtered_df)
                    st.markdown(render_custom_analysis_table(aggregated_df), unsafe_allow_html=True)
                else:
