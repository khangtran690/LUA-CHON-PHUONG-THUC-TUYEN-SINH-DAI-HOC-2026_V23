    ...
    if st.button(t["analysis_btn"]):
        with st.spinner(t["analysis_searching"]):
            ...
            if df_dh is not None and not df_dh.empty:
                req_cols = ["Tên trường", "Tên ngành", "Mã ngành", "Điểm chuẩn", "Phương thức xét tuyển"]
                available_cols = [c for c in req_cols if c in df_dh.columns]
                
                if len(available_cols) >= 2:
                    df_dh['search_text'] = df_dh.apply(lambda r: ' '.join([str(v) for v in r.values if pd.notna(v)]), axis=1)
                    matched_df = df_dh[df_dh['search_text'].apply(lambda x: check_row_match(x, selected_attr))].copy()
                    
                    if not matched_df.empty:
                        for col in available_cols:
                            matched_df[col] = matched_df[col].apply(clean_str)
                            
                        group_cols = [c for c in ["Tên trường", "Tên ngành", "Mã ngành"] if c in available_cols]
                        
                        if group_cols:
                            agg_dict = {}
                            if "Điểm chuẩn" in available_cols:
                                agg_dict["Điểm chuẩn"] = get_best_diem_chuan
                            if "Phương thức xét tuyển" in available_cols:
                                agg_dict["Phương thức xét tuyển"] = aggregate_ptxt
                                
                            df_final = matched_df.groupby(group_cols, as_index=False).agg(agg_dict)
                        else:
                            df_final = matched_df[available_cols]

                        df_final.insert(0, "STT", range(1, len(df_final) + 1))
                        
                        st.markdown(f"### {t['analysis_table_title'].format(selected_attr)}")
                        st.markdown(render_custom_analysis_table(df_final), unsafe_allow_html=True)
                    else:
