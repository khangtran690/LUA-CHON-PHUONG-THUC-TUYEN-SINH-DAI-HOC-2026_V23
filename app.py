...
TRANS = {
    "vi": {
        ...
        "um_status_suspended": "⛔ TẠM NGƯỜI HOẠT ĐỘNG",
        ...
        "um_btn_suspend": "🔴 TẠM NGƯỜI HOẠT ĐỘNG",
...
# ---------------------------------------------------------
# HÀM XỬ LÝ GOM NHÓM DỮ LIỆU ĐIỂM CHUẨN
# ---------------------------------------------------------
def process_diem_chuan_df(df):
    if df is None or df.empty:
        return df

    # Chuẩn hóa tên cột chính
    col_truong = next((c for c in df.columns if "trường" in c.lower() or "truong" in c.lower()), None)
    col_nganh = next((c for c in df.columns if "tên ngành" in c.lower() or "ten nganh" in c.lower() or "ngành" in c.lower()), None)
    col_manganh = next((c for c in df.columns if "mã ngành" in c.lower() or "ma nganh" in c.lower()), None)
    col_diem = next((c for c in df.columns if "chuẩn" in c.lower() or "chuan" in c.lower() or "điểm" in c.lower()), None)
    col_phuongthuc = next((c for c in df.columns if "phương thức" in c.lower() or "phuong thuc" in c.lower()), None)

    if not all([col_truong, col_nganh, col_manganh, col_diem, col_phuongthuc]):
        return df

    # Đảm bảo kiểu dữ liệu chuỗi để ghép chính xác
    group_cols = [col_truong, col_nganh, col_manganh, col_diem]
    
    def aggregate_phuong_thuc(series):
        pts = []
        for val in series:
            if pd.notna(val):
                s_val = str(val).strip()
                if s_val and s_val not in pts:
                    pts.append(s_val)
        return ", ".join(pts)

    df_grouped = df.groupby(group_cols, as_index=False, dropna=False).agg({
        col_phuongthuc: aggregate_phuong_thuc
    })

    # Giữ nguyên thứ tự cột ban đầu
    orig_cols = [c for c in df.columns if c in df_grouped.columns]
    return df_grouped[orig_cols]
...
with col1:
    ...
    with st.container(border=True):
        ...
        if danh_sach_ftu:
            ...
            ftu_best_30 = danh_sach_ftu[ftu_best_key]["diem30"]
            ftu_best_40 = danh_sach_ftu[ftu_best_key]["diem40"]
            
            st.markdown(f'<div class="optimal-badge">🏆 {t["opt_best"]}: {ftu_best_key}</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="opt-score-display">{t["lbl_scale_30"]}: {ftu_best_30:.2f} {t["pts_unit"]}</div>', unsafe_allow_html=True)
            st.markdown(f'**• {t["lbl_scale_40"]}:** `{ftu_best_40:.2f} {t["pts_unit"]}`')
            ...
            if other_ftu:
                ...
                for k, v in other_ftu.items():
                    st.write(f"- **{k}:** {t['lbl_scale_30']} = {v['diem30']:.2f} {t['pts_unit']} | {t['lbl_scale_40']} = {v['diem40']:.2f} {t['pts_unit']} ({v['mota']})")
        else:
            st.info(t["ftu_warn_empty"])
...
with col2:
    ...
    with st.container(border=True):
        ...
        if danh_sach_hust_100:
            ...
            st.info(t["hust_warn_100_empty"])
            
        st.markdown("---")
        st.caption(t["hust_method30_title"])
        if dxt_thpt_hust > 0:
...
# ---------------------------------------------------------
# MỤC PHÂN TÍCH VÀ TƯ VẤN DỮ LIỆU ĐIỂM CHUẨN
# ---------------------------------------------------------
st.markdown("---")
st.subheader(t["analysis_sec_title"])

df_diem_chuan_raw = load_diem_chuan_github_data()

if df_diem_chuan_raw is not None and not df_diem_chuan_raw.empty:
    df_diem_chuan = process_diem_chuan_df(df_diem_chuan_raw)
    
    col_attr, col_btn = st.columns([3, 1], vertical_alignment="bottom")
    
    with col_attr:
        search_kw = st.text_input(t["analysis_attr_label"], key="analysis_search_input")
        
    with col_btn:
        btn_analyze = st.button(t["analysis_btn"], key="btn_run_analysis", use_container_width=True)
        
    if btn_analyze or search_kw:
        if search_kw.strip():
            kw = search_kw.strip().lower()
            mask = df_diem_chuan.astype(str).apply(lambda row: row.str.lower().str.contains(kw).any(), axis=1)
            df_filtered = df_diem_chuan[mask]
            
            if not df_filtered.empty:
                st.markdown(f"#### {t['analysis_table_title'].format(search_kw)}")
                st.dataframe(df_filtered, use_container_width=True)
            else:
                st.warning(t["analysis_no_data"])
        else:
            st.dataframe(df_diem_chuan, use_container_width=True)
else:
    st.info(t["analysis_file_err"])