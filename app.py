        ...
        with st.spinner(t["analysis_searching"]):
            ...
            if raw_data is None or raw_data.empty:
                ...
                if not col_major:
                    ...
                    col_major = major_candidates[0] if major_candidates else raw_data.columns[1]
                if not col_code:
                    code_candidates = [c for c in raw_data.columns if "mã" in str(c).lower()]
                    col_code = code_candidates[0] if code_candidates else raw_data.columns[2]
                if not col_score:
                    score_candidates = [c for c in raw_data.columns if "điểm" in str(c).lower()]
                    col_score = score_candidates[0] if score_candidates else raw_data.columns[3]
                if not col_method:
                    method_candidates = [c for c in raw_data.columns if "phương thức" in str(c).lower() or "ptxt" in str(c).lower()]
                    col_method = method_candidates[0] if method_candidates else raw_data.columns[4]

                filtered_rows = []
                for idx, row in raw_data.iterrows():
                    search_str = f"{row[col_school]} {row[col_major]} {row[col_code]}"
                    if check_row_match(search_str, selected_attr):
                        c_school = clean_str(row[col_school])
                        c_major = clean_str(row[col_major])
                        c_code = clean_str(row[col_code])
                        c_score = clean_score(row[col_score])
                        c_method = clean_str(row[col_method])
                        ...
                        filtered_rows.append({
                            ...
                            "Mã ngành": c_code,
                            "Điểm chuẩn": c_score,
                            "Phương thức xét tuyển": c_method
                        })

                if filtered_rows:
                    df_res = pd.DataFrame(filtered_rows)
                    
                    # Gom nhóm các dòng có 4 cột giống nhau, hợp nhất các Phương thức xét tuyển
                    group_cols = ["Tên trường", "Tên ngành", "Mã ngành", "Điểm chuẩn"]
                    
                    aggregated_df = df_res.groupby(group_cols, as_index=False, sort=False).agg({
                        "Phương thức xét tuyển": aggregate_ptxt
                    })

                    # Đảm bảo các thuộc tính không bị trống
                    aggregated_df["Tên trường"] = aggregated_df["Tên trường"].apply(clean_str)
                    aggregated_df["Tên ngành"] = aggregated_df["Tên ngành"].apply(clean_str)
                    aggregated_df["Mã ngành"] = aggregated_df["Mã ngành"].apply(clean_str)
                    aggregated_df["Điểm chuẩn"] = aggregated_df["Điểm chuẩn"].apply(clean_score)
                    aggregated_df["Phương thức xét tuyển"] = aggregated_df["Phương thức xét tuyển"].apply(clean_str)

                    # Thêm cột STT ở vị trí đầu tiên
                    aggregated_df.insert(0, 'STT', range(1, len(aggregated_df) + 1))

                    st.markdown(f"#### {t['analysis_table_title'].format(selected_attr)}")
                    html_table = render_custom_analysis_table(aggregated_df)
