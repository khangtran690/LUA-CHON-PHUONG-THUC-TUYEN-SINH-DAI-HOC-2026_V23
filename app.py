...
# Từ điển chuyển đổi ngôn ngữ đầy đủ (Giao diện + File PDF + User Management + Phân tích & Tư vấn)
TRANS = {
    "vi": {
        ...
        # Tiêu đề & Cảnh báo Phương thức xét tuyển
        ...
        # PDF Export Dictionary (Vietnamese)
        ...
        # User Management Dictionary (Vietnamese)
        ...
        "analysis_table_title": "Danh sách các chương trình/ngành {} bạn quan tâm",
        ...
        "analysis_file_err": "❌ Không thể tải tệp dữ liệu 'DIEM CHUAN 2026' từ Github repository!"
    },
    "en": {
        ...
        # User Management Dictionary (English)
        ...
        "analysis_table_title": "List of programs/majors in {} of interest",
        "analysis_searching": "Searching program/major data...",
        "analysis_no_data": "No matching data found for the selected property.",
        "analysis_file_err": "❌ Could not load 'DIEM CHUAN 2026' data file from Github repository!"
    }
}

# Khởi tạo trạng thái ngôn ngữ & chỉ số lưu trữ selectbox
...
# URL cố định file Dữ liệu Điểm chuẩn 2026 upload trên Github (Raw URL)
GITHUB_DIEM_CHUAN_URL = "https://raw.githubusercontent.com/kadentran/kadenunilook/main/DIEM_CHUAN_2026.csv"
...
# ---------------------------------------------------------
# HÀM LẤY DỮ LIỆU TỪ FILE "DIEM CHUAN 2026" TRÊN GITHUB
# ---------------------------------------------------------
@st.cache_data(ttl=300, show_spinner=False)
def load_diem_chuan_github_data():
    """Tải dữ liệu từ tệp 'DIEM CHUAN 2026' (hoặc DIEM_CHUAN_2026.csv) từ repo Github"""
    try:
        df = pd.read_csv(GITHUB_DIEM_CHUAN_URL)
        return df
    except Exception:
        # Fallback thử đọc file local nếu có cùng thư mục
        local_files = ["DIEM_CHUAN_2026.csv", "DIEM CHUAN 2026.csv", "DIEM CHUAN 2026"]
...
# CSS tùy chỉnh giao diện và đường viền hiện đại cho 4 phân khu
st.markdown(f"""
    <style>
    .copyright-header {{
        position: absolute;
    ...
    /* CSS nâng cấp đường viền 4 phân khu hiện đại & bắt mắt */
    ...
    /* Huy hiệu Optimal Choice đồng bộ */
    .optimal-badge {{
        display: inline-flex;
        align-items: center;
        gap: 4px;
...
# Hàm Callback giúp chuyển đổi ngôn ngữ phản hồi tức thì 1-click
def on_lang_change():
...
class DashboardPDF(FPDF):
    ...
    def header(self):
        ...
        self.set_xy(10, 6)
        self.cell(0, 10, self.header_title, ln=True, align="L")
...
def generate_pdf(ho_ten, so_cccd, sat, ielts, vact, hsa, tsa, is_chuyen, hsg_tinh, hsg_quocgia, 
    ...
    pdf.cell(0, 6, t_pdf["pdf_sec1"], ln=True)
    pdf.ln(2)
    ...
    def draw_grid_row(y_pos, row_title, items):
        ...
        pdf.cell(190, 4, row_title, ln=True)
        ...
        for idx, (label, val) in enumerate(items):
            ...
            pdf.cell(item_width - 2, 3.5, lbl_text, align="C", ln=True)
    ...
    pdf.set_font("DejaVu", "B", 10.5)
    pdf.set_text_color(30, 58, 138)
    pdf.cell(0, 6, t_pdf["pdf_sec2"], ln=True)
    pdf.ln(1)
    ...
    def draw_school_card(title, data_dict, is_ftu=False, extra_hust=0.0):
        ...
        pdf.set_text_color(15, 23, 42)
        pdf.cell(190, 5.5, f"  {title}", ln=True, fill=True)
...
with col1:
    with st.container(border=True):
        ...
        if danh_sach_ueh:
            ...
            ueh_best_val = danh_sach_ueh[ueh_best_key]
            
            st.markdown(f'<div class="optimal-badge">🏆 {t["opt_best"]}: {ueh_best_key}</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="opt-score-display">{ueh_best_val["diem"]:.2f} <span style="font-size: 1.2rem; color: #64748b;">/ 100 {t["pts_unit"]}</span></div>', unsafe_allow_html=True)
            st.caption(f"• {t['lbl_exam_source']}: **{ueh_best_val['mota']}** | {t['lbl_gpa_thpt']}: **{diem_tb_thpt_ueh:.1f}/100** | {t['lbl_bonus_pts']}: **{diem_cong_ueh:.1f}** | {t['lbl_priority_pts']}: **{diem_ut:.2f}**")
            
            other_ueh = {k: v for k, v in danh_sach_ueh.items() if k != ueh_best_key}
            if other_ueh:
                st.markdown(f"**{t['opt_comparison']}**")
                for k, v in other_ueh.items():
                    st.write(f"• **{k}:** `{v['diem']:.2f}` / 100 {t['pts_unit']} *(Nguồn: {v['mota']})*")
        else:
...
with col2:
    with st.container(border=True):
        ...
        if danh_sach_doi_tuong:
            hcmut_best_key = max(danh_sach_doi_tuong, key=lambda k: danh_sach_doi_tuong[k]["diem"])
            hcmut_best_val = danh_sach_doi_tuong[hcmut_best_key]
            
            st.markdown(f'<div class="optimal-badge">🏆 {t["opt_best"]}: {hcmut_best_key}</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="opt-score-display">{hcmut_best_val["diem"]:.2f} <span style="font-size: 1.2rem; color: #64748b;">/ 100 {t["pts_unit"]}</span></div>', unsafe_allow_html=True)
            st.caption(f"• {t['lbl_aptitude_used']}: **{hcmut_best_val['mota']}** | {t['lbl_thpt_exam_20']}: **{thpt_quydoi_bk:.1f}** | {t['lbl_gpa_10']}: **{gpa_quydoi_bk:.1f}** | {t['lbl_bonus_pts']}: **{diem_cong_bk:.1f}** | {t['lbl_priority_pts']}: **{diem_ut:.2f}**")
            
            other_hcmut = {k: v for k, v in danh_sach_doi_tuong.items() if k != hcmut_best_key}
            if other_hcmut:
                st.markdown(f"**{t['opt_comparison']}**")
                for k, v in other_hcmut.items():
                    st.write(f"• **{k}:** `{v['diem']:.2f}` / 100 {t['pts_unit']} *({v['mota']})*")
        else:
...
with col3:
    with st.container(border=True):
        ...
        if danh_sach_ftu:
            ...
            ftu_best_val = danh_sach_ftu[ftu_best_key]
            
            st.markdown(f'<div class="optimal-badge">🏆 {t["opt_best"]}: {ftu_best_key}</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="opt-score-display">{ftu_best_val["diem30"]:.2f} <span style="font-size: 1.2rem; color: #64748b;">/ 30 {t["pts_unit"]} &nbsp;|&nbsp; {ftu_best_val["diem40"]:.2f} / 40 {t["pts_unit"]}</span></div>', unsafe_allow_html=True)
            st.caption(f"• {t['lbl_scoring_detail']}: **{ftu_best_val['mota']}** | {t['lbl_priority_pts']}: **{diem_ut:.2f}** (Thang 30) / **{diem_ut_40:.2f}** (Thang 40)")
            
            other_ftu = {k: v for k, v in danh_sach_ftu.items() if k != ftu_best_key}
            if other_ftu:
                st.markdown(f"**{t['opt_comparison']}**")
                for k, v in other_ftu.items():
                    st.write(f"• **{k}:** {t['lbl_scale_30']} = `{v['diem30']:.2f}` | {t['lbl_scale_40']} = `{v['diem40']:.2f}` *({v['mota']})*")
        else:
...
with col4:
    with st.container(border=True):
        ...
        if st.session_state.hsg_quocgia_idx in [1, 2]:
            st.success(t["direct_admission_eligible"])
            
        if danh_sach_hust_100:
            hust_best_key = max(danh_sach_hust_100, key=lambda k: danh_sach_hust_100[k]["diem"])
            hust_best_val = danh_sach_hust_100[hust_best_key]
            
            st.markdown(f'<div class="optimal-badge">🏆 {t["opt_best"]}: {hust_best_key}</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="opt-score-display">{hust_best_val["diem"]:.2f} <span style="font-size: 1.2rem; color: #64748b;">/ 100 {t["pts_unit"]}</span></div>', unsafe_allow_html=True)
            st.caption(f"• {t['lbl_method_detail']}: **{hust_best_val['mota']}**")
...
# ---------------------------------------------------------
# CỦNG CỐ & THAY THẾ TÍNH NĂNG: PHÂN TÍCH & TƯ VẤN
# ---------------------------------------------------------
st.markdown("---")
st.header(t["analysis_sec_title"])

# Dánh sách 5 thuộc tính cố định theo yêu cầu
analysis_properties = [
    "Trí tuệ nhân tạo (AI) trong kinh doanh",
    "Khoa học dữ liệu trong kinh doanh",
    "Phân tích dữ liệu trong kinh doanh",
    "Khoa học máy tính trong kinh doanh",
    "Giảng dạy & học tập toàn diện bằng Tiếng Anh (các ngành trên)"
]

selected_attr = st.selectbox(
    t["analysis_attr_label"],
    options=analysis_properties,
    key="selectbox_analysis_property"
)

btn_analyze = st.button(t["analysis_btn"], key="btn_run_analysis", type="primary")

if btn_analyze:
    with st.spinner(t["analysis_searching"]):
        df_diem_chuan = load_diem_chuan_github_data()
        
        if df_diem_chuan is None or df_diem_chuan.empty:
            st.error(t["analysis_file_err"])
        else:
            # Xác định từ khóa để lọc dữ liệu tương ứng với từng thuộc tính
            keywords_map = {
                "Trí tuệ nhân tạo (AI) trong kinh doanh": ["trí tuệ nhân tạo", "ai", "artificial intelligence"],
                "Khoa học dữ liệu trong kinh doanh": ["khoa học dữ liệu", "data science"],
                "Phân tích dữ liệu trong kinh doanh": ["phân tích dữ liệu", "data analytics", "business analytics"],
                "Khoa học máy tính trong kinh doanh": ["khoa học máy tính", "computer science"],
                "Giảng dạy & học tập toàn diện bằng Tiếng Anh (các ngành trên)": ["tiếng anh", "chất lượng cao", "toàn bằng tiếng anh", "english", "tiên tiến"]
            }
            
            target_keywords = keywords_map.get(selected_attr, [])
            
            # Chuẩn hóa tên cột trong dataframe để tìm kiếm linh hoạt
            df_working = df_diem_chuan.copy()
            df_working.columns = [str(c).strip() for c in df_working.columns]
            
            # Lọc theo keyword trong tất cả các cột kiểu chuỗi
            mask = pd.Series(False, index=df_working.index)
            for col in df_working.columns:
                col_str = df_working[col].astype(str).str.lower()
                for kw in target_keywords:
                    mask = mask | col_str.str.contains(kw.lower(), na=False)
            
            filtered_df = df_working[mask].copy()
            
            # Tiêu đề bảng theo yêu cầu
            table_title = t["analysis_table_title"].format(selected_attr)
            st.markdown(f"### {table_title}")
            
            if filtered_df.empty:
                st.info(t["analysis_no_data"])
            else:
                # Tìm kiếm và ghép nối các cột về đúng cấu trúc chuẩn: Tên trường, Mã, Điểm chuẩn, Phương thức trúng tuyển
                def find_col_name(candidates, default=""):
                ...
                col_truong = find_col_name(["trường", "ten truong", "tên trường", "school", "university"], default=df_working.columns[0])
                col_ma = find_col_name(["mã ngành", "ma nganh", "mã", "code"], default=df_working.columns[1] if len(df_working.columns) > 1 else df_working.columns[0])
                col_diem = find_col_name(["điểm chuẩn", "diem chuan", "điểm", "benchmark", "score"], default=df_working.columns[2] if len(df_working.columns) > 2 else df_working.columns[0])
                col_phuongthuc = find_col_name(["phương thức", "phuong thuc", "phương thức trúng tuyển", "method"], default=df_working.columns[3] if len(df_working.columns) > 3 else df_working.columns[0])
                ...
                for _, row in filtered_df.iterrows():
                    result_rows.append({
                        "STT": stt_counter,
                        "Tên trường": str(row.get(col_truong, "N/A")),
                        "Mã": str(row.get(col_ma, "N/A")),
                        "Điểm chuẩn": str(row.get(col_diem, "N/A")),
                        "Phương thức trúng tuyển": str(row.get(col_phuongthuc, "N/A"))
                    })
                    stt_counter += 1

                df_result = pd.DataFrame(result_rows)

                # HTML custom table với tiêu đề cột bold, màu sắc khác biệt nổi bật
                html_table = """
                <style>
                    .custom-table {
                        width: 100%;
                ...
                <table class="custom-table">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 17%;">Điểm chuẩn</th>
                            <th style="width: 25%;">Phương thức trúng tuyển</th>
                        </tr>
                    </thead>
                    <tbody>
                """
                for row in result_rows:
                    html_table += f"""
                        <tr>
                            <td style="text-align: center; font-weight: bold;">{row['STT']}</td>
                            <td>{row['Tên trường']}</td>
                            <td>{row['Mã']}</td>
                            <td style="font-weight: bold; color: #1e3a8a;">{row['Điểm chuẩn']}</td>
                            <td>{row['Phương thức trúng tuyển']}</td>
                        </tr>
                    """
