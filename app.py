...
def process_table_aggregation(df_input):
    ...
    # Gom nhóm dữ liệu trùng 4 trường, nối cột 'Phương thức xét tuyển'
    ...
    if st.button(t["analysis_btn"]):
        with st.spinner(t["analysis_searching"]):
            ...
            if df_dh is not None and not df_dh.empty:
                # Lọc hàng theo thuộc tính
                mask = df_dh.apply(lambda r: check_row_match(" ".join([str(v) for v in r.values]), selected_attr), axis=1)
                filtered_df = df_dh[mask].copy()
                
                if not filtered_df.empty:
                    # Gom nhóm theo quy luật yêu cầu
                    processed_df = process_table_aggregation(filtered_df)
                    st.markdown(f"#### {t['analysis_table_title'].format(selected_attr)}")
                    st.markdown(render_custom_analysis_table(processed_df), unsafe_allow_html=True)
                else:
