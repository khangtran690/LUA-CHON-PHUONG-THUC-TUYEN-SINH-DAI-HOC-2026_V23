...
thpt_mon3 = st.sidebar.number_input(t["thpt_sub3"], min_value=0.0, max_value=10.0, step=0.25, key="val_thpt_sub3", disabled=disabled_input)
        ...
        with st.spinner(t["analysis_searching"]):
            ...
            if raw_data is None or raw_data.empty:
                ...
                if not matched_rows:
                    ...
                    result_data = []
                    for idx, r in enumerate(df_matched.iterrows(), 1):
                        row_data = r[1]
                        school_val = clean_str(row_data[col_school]) if col_school in df_matched.columns else "N/A"
                        major_val = clean_str(row_data[col_major]) if col_major in df_matched.columns else "N/A"
                        code_val = clean_str(row_data[col_code]) if col_code and col_code in df_matched.columns else "N/A"
                        score_val = clean_score(row_data[col_score]) if col_score and col_score in df_matched.columns else "N/A"
                        
                        if col_method and col_method in df_matched.columns:
                            method_val = aggregate_ptxt([row_data[col_method]])
                        else:
                            other_vals = [row_data[c] for c in df_matched.columns if c not in [col_school, col_major, col_code, col_score]]
                            method_val = aggregate_ptxt(other_vals)

                        result_data.append({
                            "STT": idx,
                            "Tên trường": school_val,
                            "Tên ngành": major_val,
                            "Mã ngành": code_val,
                            "Điểm chuẩn": score_val,
                            "Phương thức xét tuyển": method_val
                        })
