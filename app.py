...
TRANS = {
    "vi": {
        ...
        "analysis_file_err": "❌ Không thể tải tệp dữ liệu 'DIEM_CHUAN_2026' từ Github repository!"
    },
    "en": {
        ...
        "analysis_file_err": "❌ Could not load 'DIEM_CHUAN_2026' data file from Github repository!"
    }
}
...
# URL cố định file Dữ liệu Điểm chuẩn 2026 upload trên Github (Raw URL) - Cập nhật đúng file DIEM_CHUAN_2026.xlsx
...
# ---------------------------------------------------------
# HÀM LẤY DỮ LIỆU TỪ FILE "DIEM_CHUAN_2026.xlsx" TRÊN GITHUB
# ---------------------------------------------------------
@st.cache_data(ttl=300, show_spinner=False)
def load_diem_chuan_github_data():
    """Tải dữ liệu từ tệp 'DIEM_CHUAN_2026.xlsx' (hoặc fallback CSV / Local File)"""
    # 1. Thử tải file Excel từ URL GitHub
    try:
        df = pd.read_excel(GITHUB_DIEM_CHUAN_XLSX_URL, engine="openpyxl")
        return df
    except Exception:
        pass
        
    # 2. Fallback thử tải file CSV từ URL GitHub
    try:
        df = pd.read_csv(GITHUB_DIEM_CHUAN_CSV_URL)
        return df
    except Exception:
        pass

    # 3. Fallback đọc từ tệp local
    local_files = [
