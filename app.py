    ...
    if st.button(t["analysis_btn"]):
        with st.spinner(t["analysis_searching"]):
            ...
            if raw_data is None or raw_data.empty:
                ...
                if not col_method:
                    method_candidates = [c for c in raw_data.columns if "phương" in str(c).lower() or "ptxt" in str(c).lower()]
                    col_method = method_candidates[0] if method_candidates else None

                filtered_rows = []
                for _, row in raw_data.iterrows():
                    major_val = row[col_major] if col_major and col_major in raw_data.columns else ""
                    if check_row_match(major_val, selected_attr):
                ...
                if not filtered_rows:
                    ...
                    for _, row in df_filtered.iterrows():
                        school_val = clean_str(row[col_school]) if col_school and col_school in df_filtered.columns else "N/A"
                        major_val = clean_str(row[col_major]) if col_major and col_major in df_filtered.columns else "N/A"
                        code_val = clean_str(row[col_code]) if col_code and col_code in df_filtered.columns else "N/A"
                        score_val = clean_score(row[col_score]) if col_score and col_score in df_filtered.columns else "N/A"
                        method_val = clean_str(row[col_method]) if col_method and col_method in df_filtered.columns else "N/A"
                        
                        display_data.append({
                            "STT": len(display_data) + 1,
                    ...
                    st.markdown(render_custom_analysis_table(df_display), unsafe_allow_html=True)