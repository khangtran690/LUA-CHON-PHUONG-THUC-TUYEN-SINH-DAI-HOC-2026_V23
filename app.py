...
with col1:
    with st.container(border=True):
        ...
        if danh_sach_ueh:
            ...
            st.markdown(f'<div class="opt-score-display">{ueh_best_val:.2f} <span style="font-size: 1.1rem; font-weight: normal; color: #64748b;">{t["pts_unit"]} (Thang 100)</span></div>', unsafe_allow_html=True)
...
with col2:
    with st.container(border=True):
        ...
        if danh_sach_doi_tuong:
            ...
            st.markdown(f'<div class="opt-score-display">{bk_best_val:.2f} <span style="font-size: 1.1rem; font-weight: normal; color: #64748b;">{t["pts_unit"]} (Thang 100)</span></div>', unsafe_allow_html=True)
...
with col4:
    with st.container(border=True):
        ...
        if danh_sach_hust_100:
            ...
            st.markdown(f'<div class="opt-score-display">{hust_best_val:.2f} <span style="font-size: 1.1rem; font-weight: normal; color: #64748b;">{t["pts_unit"]} (Thang 100)</span></div>', unsafe_allow_html=True)
    ...
    if st.button(t["analysis_btn"]):
        with st.spinner(t["analysis_searching"]):
            ...
            if raw_data is None or raw_data.empty:
                ...
                if not col_school:
                    school_candidates = [c for c in raw_data.columns if "trường" in str(c).lower()]
                    col_school = school_candidates[0] if school_candidates else raw_data.columns[0]
                if not col_major:
                    major_candidates = [c for c in raw_data.columns if "ngành" in str(c).lower() and "tên" in str(c).lower()]
                    if not major_candidates:
                        major_candidates = [c for c in raw_data.columns if "ngành" in str(c).lower()]
                    col_major = major_candidates[0] if major_candidates else (raw_data.columns[1] if len(raw_data.columns) > 1 else raw_data.columns[0])
                if not col_code:
                    code_candidates = [c for c in raw_data.columns if "mã" in str(c).lower()]
                    col_code = code_candidates[0] if code_candidates else None
                if not col_score:
                    score_candidates = [c for c in raw_data.columns if "điểm" in str(c).lower() or "score" in str(c).lower()]
                    col_score = score_candidates[0] if score_candidates else None
                if not col_method:
                    method_candidates = [c for c in raw_data.columns if "phương thức" in str(c).lower() or "ptxt" in str(c).lower() or "method" in str(c).lower()]
                    col_method = method_candidates[0] if method_candidates else None

                matched_rows = []
                for _, row in raw_data.iterrows():
                    row_text = " ".join([str(val) for val in row.values if pd.notna(val)])
                    if check_row_match(row_text, selected_attr):
                ...
                if not matched_rows:
                    ...
                    score_col_candidates = [c for c in df_matched.columns if any(k in str(c).lower() for k in ["điểm chuẩn", "diem chuan", "điểm trúng tuyển", "diem trung tuyen", "score"])]
                    primary_score_col = col_score if col_score in df_matched.columns else (score_col_candidates[0] if score_col_candidates else None)
                    
                    school_col_candidates = [c for c in df_matched.columns if any(k in str(c).lower() for k in ["tên trường", "trường", "truong", "university"])]
                    primary_school_col = col_school if col_school in df_matched.columns else (school_col_candidates[0] if school_col_candidates else df_matched.columns[0])
                    
                    major_col_candidates = [c for c in df_matched.columns if any(k in str(c).lower() for k in ["tên ngành", "ngành", "major"])]
                    primary_major_col = col_major if col_major in df_matched.columns else (major_col_candidates[0] if major_col_candidates else df_matched.columns[1] if len(df_matched.columns) > 1 else df_matched.columns[0])
                    
                    code_col_candidates = [c for c in df_matched.columns if any(k in str(c).lower() for k in ["mã ngành", "mã", "code"])]
                    primary_code_col = col_code if col_code in df_matched.columns else (code_col_candidates[0] if code_col_candidates else None)
                    
                    method_col_candidates = [c for c in df_matched.columns if any(k in str(c).lower() for k in ["phương thức", "ptxt", "method"])]
                    primary_method_col = col_method if col_method in df_matched.columns else (method_col_candidates[0] if method_col_candidates else None)

                    for idx, (_, r) in enumerate(df_matched.iterrows(), 1):
                        s_name = clean_str(r[primary_school_col]) if primary_school_col in r and pd.notna(r[primary_school_col]) else "N/A"
                        m_name = clean_str(r[primary_major_col]) if primary_major_col in r and pd.notna(r[primary_major_col]) else "N/A"
                        m_code = clean_str(r[primary_code_col]) if primary_code_col and primary_code_col in r and pd.notna(r[primary_code_col]) else "N/A"
                        
                        sc_val = "N/A"
                        if primary_score_col and primary_score_col in r and pd.notna(r[primary_score_col]):
                            sc_val = clean_score(r[primary_score_col])
                        
                        if sc_val == "N/A" or sc_val == "":
                            for sc_c in score_col_candidates:
                                if sc_c in r and pd.notna(r[sc_c]):
                                    val_try = clean_score(r[sc_c])
                                    if val_try != "N/A" and val_try != "":
                                        sc_val = val_try
                                        break
                        
                        if sc_val == "N/A" or sc_val == "":
                            for col_name, val in r.items():
                                if any(k in str(col_name).lower() for k in ["điểm", "diem", "score"]) and pd.notna(val):
                                    val_try = clean_score(val)
                                    if val_try != "N/A" and val_try != "":
                                        sc_val = val_try
                                        break

                        pt_val = "N/A"
                        if primary_method_col and primary_method_col in r and pd.notna(r[primary_method_col]):
                            pt_val = aggregate_ptxt([r[primary_method_col]])
                        if pt_val == "N/A":
                            for m_c in method_col_candidates:
                                if m_c in r and pd.notna(r[m_c]):
                                    pt_try = aggregate_ptxt([r[m_c]])
                                    if pt_try != "N/A":
                                        pt_val = pt_try
                                        break
                        ...
                        result_data.append({
                            ...
                            "Mã ngành": m_code,
                            "Điểm chuẩn": sc_val,
                            "Phương thức xét tuyển": pt_val
                        })
                    
                    df_result = pd.DataFrame(result_data)
                    st.markdown(f"### {t['analysis_table_title'].format(selected_attr)}")
                    st.markdown(render_custom_analysis_table(df_result), unsafe_allow_html=True)