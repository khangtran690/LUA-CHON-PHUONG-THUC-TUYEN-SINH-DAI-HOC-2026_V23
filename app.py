...
with st.container(border=True):
    ...
    def check_row_match(row_text, attr):
        ...
        if attr == "Trí tuệ nhân tạo (AI) trong kinh doanh":
            ...
            english_kws = ["tiên tiến", "ct tt", "tiếng anh", "chất lượng cao", "clc", "quốc tế", "english", "tieng anh", "advanced"]
            match_eng = any(kw in text for kw in english_kws)
            return match_base and match_eng
        return False

    if st.button(t["analysis_btn"], key="btn_do_analysis"):
        with st.spinner(t["analysis_searching"]):
            df_dh = load_dh_2026_data()
            if df_dh is not None and not df_dh.empty:
                cols = df_dh.columns.tolist()
                col_truong = next((c for c in cols if c in ["Tên trường", "Trường"]), None)
                col_nganh = next((c for c in cols if c in ["Tên ngành", "Ngành"]), None)
                col_ma = next((c for c in cols if c in ["Mã ngành", "MãNgành"]), None)
                col_diem = next((c for c in cols if "điểm" in str(c).lower() or "diem" in str(c).lower()), None)
                col_ptxt = next((c for c in cols if "phương thức" in str(c).lower() or "ptxt" in str(c).lower()), None)

                matched_rows = []
                for idx, row in df_dh.iterrows():
                    search_str = " ".join([str(val) for val in row.values if pd.notna(val)])
                    if check_row_match(search_str, selected_attr):
                        matched_rows.append({
                            "Tên trường": clean_str(row[col_truong]) if col_truong else "N/A",
                            "Tên ngành": clean_str(row[col_nganh]) if col_nganh else "N/A",
                            "Mã ngành": clean_str(row[col_ma]) if col_ma else "N/A",
                            "Điểm chuẩn": clean_score(row[col_diem]) if col_diem else "N/A",
                            "Phương thức xét tuyển": clean_str(row[col_ptxt]) if col_ptxt else "N/A"
                        })

                if matched_rows:
                    df_matched = pd.DataFrame(matched_rows)
                    grouped = df_matched.groupby(["Tên trường", "Tên ngành", "Mã ngành"], as_index=False).agg({
                        "Điểm chuẩn": get_best_diem_chuan,
                    ...
                    st.markdown(render_custom_analysis_table(grouped), unsafe_allow_html=True)
                else:
