...
TRANS = {
    "vi": {
        ...
        "login_err_inactive": "Tài khoản của bạn đã bị TẠM NGƯỜNG HOẠT ĐỘNG!",
        ...
        "um_status_suspended": "⛔ TẠM NGƯỜNG HOẠT ĐỘNG",
        ...
        "analysis_attr_label": "Chọn thuộc tính quan tâm (Tên trường, ngành, mã...):",
    ...
    "en": {
        ...
        "analysis_attr_label": "Select property of interest (School, major, code...):",
        ...
        "analysis_file_err": "❌ Unable to load data file 'DH_2026.xlsx' from Github repository or local file!"
    }
}
...
def load_dh_2026_data():
    ...
    if df_result is None or df_result.empty:
        try:
            ...
            if r.status_code == 200:
                df_result = pd.read_csv(io.StringIO(r.content.decode('utf-8')))
        except Exception:
            pass
    ...
    if df_result is not None and not df_result.empty:
        ...
        for c in df_result.columns:
            c_str = str(c).strip()
            c_lower = c_str.lower()
            if "trường" in c_lower or "truong" in c_lower:
                col_map[c] = "Tên trường"
            elif "tên ngành" in c_lower or "ngành" in c_lower or "nganh" in c_lower:
                col_map[c] = "Tên ngành"
            elif "mã ngành" in c_lower or "ma ngành" in c_lower or "ma_nganh" in c_lower:
                col_map[c] = "Mã ngành"
            elif "điểm chuẩn" in c_lower or "diem_chuan" in c_lower or "diem chuan" in c_lower or "điểm trúng tuyển" in c_lower:
                col_map[c] = "Điểm chuẩn"
            elif "phương thức" in c_lower or "ptxt" in c_lower or "phuong thuc" in c_lower:
                col_map[c] = "Phương thức xét tuyển"
...
def get_best_diem_chuan(series):
    valid_scores = []
    for val in series:
        s = clean_score(val)
        if s != "N/A":
...
def render_custom_analysis_table(df):
    ...
    html_code += '<th>STT</th><th>Trường</th><th>Ngành học</th><th>Mã ngành</th><th>Điểm chuẩn</th><th>Phương thức xét tuyển</th></tr></thead><tbody>'
    
    for idx, (_, row) in enumerate(df.iterrows(), 1):
        truong = html.escape(str(row.get("Tên trường", "N/A")))
        nganh = html.escape(str(row.get("Tên ngành", "N/A")))
        ma = html.escape(str(row.get("Mã ngành", "N/A")))
        diem = html.escape(clean_score(row.get("Điểm chuẩn", "N/A")))
        ptxt = html.escape(str(row.get("Phương thức xét tuyển", "N/A")))
        
        html_code += f'<tr><td class="col-stt-cell">{idx}</td>'
        html_code += f'<td class="col-truong-cell">{truong}</td>'
        html_code += f'<td class="col-nganh-cell">{nganh}</td>'
        html_code += f'<td class="col-ma-cell">{ma}</td>'
        html_code += f'<td class="col-diem-cell">{diem}</td>'
        html_code += f'<td class="col-ptxt-cell">{ptxt}</td></tr>'
        
    html_code += '</tbody></table></div>'
    return html_code

# Khởi tạo giao diện cho phần Phân tích
st.markdown("---")
st.subheader(t["analysis_sec_title"])

df_2026 = load_dh_2026_data()
if df_2026 is not None and not df_2026.empty:
    col_search, col_btn = st.columns([4, 1], vertical_alignment="bottom")
    with col_search:
        search_query = st.text_input(t["analysis_attr_label"])
    with col_btn:
        btn_search = st.button(t["analysis_btn"], use_container_width=True)
    ...
    if btn_search or search_query:
        with st.spinner(t["analysis_searching"]):
            ...
            if search_query:
                ...
                # Tìm kiếm chuỗi (không phân biệt hoa thường) trên tất cả các cột dữ liệu dạng chuỗi
                mask = df_filtered.astype(str).apply(lambda x: x.str.lower().str.contains(search_lower, na=False)).any(axis=1)
                df_filtered = df_filtered[mask]
            
            if not df_filtered.empty:
                st.markdown(f"**{t['analysis_table_title'].format(search_query if search_query else 'Tất cả')}**")
                html_table = render_custom_analysis_table(df_filtered)
