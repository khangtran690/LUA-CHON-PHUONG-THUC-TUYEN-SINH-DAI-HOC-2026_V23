...
TRANS = {
    "vi": {
        ...
        "analysis_file_err": "❌ Không thể tải tệp dữ liệu 'DIEM_CHUAN_2026' từ Github repository hoặc file local!"
    },
    "en": {
        ...
        "analysis_file_err": "❌ Unable to load 'DIEM_CHUAN_2026' data file from Github repository or local file!"
    }
}
...
# ---------------------------------------------------------
# HÀM LẤY DỮ LIỆU TỪ FILE "DIEM_CHUAN_2026.xlsx"
# ---------------------------------------------------------
        ...
        if df_diem_chuan is None or df_diem_chuan.empty:
            ...
            for col in df_working.columns:
                ...
                for kw in target_keywords:
                    # KHẮC PHỤC LỖI TÌM KIẾM TỪ KHÓA "AI":
                    # Thay thế các ký tự ngoặc, gạch ngang, dấu câu bằng khoảng trắng,
                    # và đệm thêm khoảng trắng 2 bên để tách biệt chính xác từ " ai "
                    if kw == "ai":
                        padded_col = " " + col_str.str.replace(r'[.,;!?"()\[\]{}-]', ' ', regex=True) + " "
                        mask = mask | padded_col.str.contains(" ai ", regex=False, na=False)
                    else:
                        mask = mask | col_str.str.contains(kw.lower(), regex=False, na=False)
            ...
            if filtered_df.empty:
                ...
                def find_col_name(candidates, default=""):
                    for cand in candidates:
                        for c in df_working.columns:
                            if cand.lower() == c.lower(): return c
                    for cand in candidates:
                        for c in df_working.columns:
                ...
                col_nganh = find_col_name(["tên ngành", "ngành", "chuyên ngành", "major"], default=df_working.columns[2] if len(df_working.columns) > 2 else df_working.columns[0])
                col_diem = find_col_name(["điểm chuẩn", "diem chuan", "điểm", "benchmark", "score"], default=df_working.columns[3] if len(df_working.columns) > 3 else df_working.columns[0])
                col_phuongthuc = find_col_name(["phương thức", "phuong thuc", "phương thức trúng tuyển", "method"], default=df_working.columns[4] if len(df_working.columns) > 4 else df_working.columns[0])
                ...
                for _, row in filtered_df.iterrows():
                    result_rows.append({
                        ...
                        "Tên ngành": str(row.get(col_nganh, "N/A")),
                ...
                <table class="custom-analysis-table">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 12%;">Điểm chuẩn</th>
                            <th style="width: 23%;">Phương thức trúng tuyển</th>
                        </tr>
                    </thead>
                    <tbody>
                """
                for row in result_rows:
                    html_table += f"""
                        <tr>
                            ...
                            <td>{row['Mã']}</td>
                            <td style="font-weight: bold; color: #1e40af;">{row['Tên ngành']}</td>
