...
# ---------------------------------------------------------
# MỤC PHÂN TÍCH VÀ TƯ VẤN DỮ LIỆU ĐIỂM CHUẨN (ĐÃ ĐỔI SANG SỬ DỤNG DH_2026.xlsx)
# ---------------------------------------------------------
st.markdown("---")
st.subheader(t["analysis_sec_title"])

df_dh_2026_raw = load_dh_2026_data()

if df_dh_2026_raw is not None and not df_dh_2026_raw.empty:
    df_dh_2026 = process_diem_chuan_df(df_dh_2026_raw)
    ...
    if btn_analyze or search_kw:
        if search_kw.strip():
            ...
            mask = df_dh_2026.astype(str).apply(lambda row: row.str.lower().str.contains(kw).any(), axis=1)
            df_filtered = df_dh_2026[mask]
            ...
            st.dataframe(df_dh_2026, use_container_width=True)
else:
