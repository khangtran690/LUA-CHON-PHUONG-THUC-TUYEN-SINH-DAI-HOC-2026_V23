...
# ---------------------------------------------------------
# PHÂN TÍCH & TƯ VẤN (TÌM KIẾM TRÊN TẤT CẢ CÁC SHEET DỮ LIỆU)
# ---------------------------------------------------------
def clean_str(val):
    ...
    if st.button(t["analysis_btn"]):
        with st.spinner(t["analysis_searching"]):
            ...
            if df_dh is None:
                ...
                col_truong, col_nganh, col_ma, col_diem, col_ptxt = None, None, None, None, None
                for c in df_dh.columns:
                    c_low = str(c).strip().lower()
                    if 'trường' in c_low or 'truong' in c_low:
                        col_truong = c
                    elif 'tên ngành' in c_low or 'ten nganh' in c_low or 'ngành' in c_low or 'nganh' in c_low:
                        if not col_nganh: col_nganh = c
                    elif 'mã ngành' in c_low or 'ma nganh' in c_low or 'mã' in c_low:
                        if not col_ma: col_ma = c
                    elif 'điểm chuẩn' in c_low or 'diem chuan' in c_low or 'điểm' in c_low:
                        if not col_diem: col_diem = c
                    elif 'ptxt' in c_low or 'phương thức' in c_low or 'phuong thuc' in c_low:
                        if not col_ptxt: col_ptxt = c

                if not col_truong or not col_nganh:
                    st.error("❌ Tệp dữ liệu không chứa đúng cấu trúc cột yêu cầu!")
                else:
                    matched_rows = []
                    for _, row in df_dh.iterrows():
                        search_target = f"{clean_str(row.get(col_truong, ''))} {clean_str(row.get(col_nganh, ''))}"
                        if check_row_match(search_target, selected_attr):
                            matched_rows.append({
                                'Tên trường': clean_str(row.get(col_truong, '')),
                                'Tên ngành': clean_str(row.get(col_nganh, '')),
                                'Mã ngành': clean_str(row.get(col_ma, '')),
                                'Điểm chuẩn': clean_score(row.get(col_diem, '')),
                                'PTXT': clean_str(row.get(col_ptxt, ''))
                            })
                    ...
                    if matched_rows:
                        ...
                        def aggregate_scores(series):
                            valid_scores = [str(s).strip() for s in series if str(s).strip() != ""]
                            seen_scores = []
                            for sc in valid_scores:
                                if sc not in seen_scores:
                                    seen_scores.append(sc)
                            return ", ".join(seen_scores) if seen_scores else "N/A"

                        df_grouped = df_result.groupby(
                            ['Tên trường', 'Tên ngành', 'Mã ngành'], as_index=False
                        ).agg({
                            'Điểm chuẩn': aggregate_scores,
                            'PTXT': aggregate_ptxt
                        })
                        ...
                        st.markdown(render_custom_analysis_table(df_grouped), unsafe_allow_html=True)
                    else:
