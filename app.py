...
# ---------------------------------------------------------
# HÀM LẤY DỮ LIỆU TỪ FILE "DH_2026.xlsx" & "DIEM_CHUAN_2026.xlsx"
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# PHÂN TÍCH & TƯ VẤN (CẬP NHẬT TÍNH NĂNG TÌM KIẾM & XUẤT BẢNG)
# ---------------------------------------------------------
st.markdown("---")
with st.container(border=True):
            ...
            if df_dh is None:
                ...
                for col in df_dh.columns:
                    ...
                    if 'trường' in c_low or 'truong' in c_low:
                        ...
                        col_mapping[col] = 'Phương thức xét tuyến'
                
                df_dh = df_dh.rename(columns=col_mapping)
                required_cols = ['Tên trường', 'Tên ngành', 'Mã ngành', 'Điểm chuẩn', 'Phương thức xét tuyến']
                ...
                for _, row in df_dh.iterrows():
                    ...
                    if check_row_match(row_full_text, selected_attr):
                        matched_rows.append({
                            ...
                            'Phương thức xét tuyến': str(row.get('Phương thức xét tuyến', ''))
                        })
                ...
                if not matched_rows:
                    ...
                    # Gộp nhóm các dòng có 4 cột giống nhau và thống kê tất cả phương thức xét tuyến
                    grouped_df = df_res.groupby(['Tên trường', 'Tên ngành', 'Mã ngành', 'Điểm chuẩn'], as_index=False).agg({
                        'Phương thức xét tuyến': lambda x: ", ".join(sorted(list(set(str(v).strip() for v in x if pd.notna(v) and str(v).strip() != ''))))
                    })
                    
                    # Chèn cột STT đầu tiên đánh số từ 1 trở đi
                    grouped_df.insert(0, 'STT', range(1, len(grouped_df) + 1))
                    
                    final_columns = ['STT', 'Tên trường', 'Tên ngành', 'Mã ngành', 'Điểm chuẩn', 'Phương thức xét tuyến']
