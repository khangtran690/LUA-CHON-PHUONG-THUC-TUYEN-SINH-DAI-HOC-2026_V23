...
def load_dh_2026_data():
    ...
    if df_result is not None and not df_result.empty:
        # Chuẩn hóa tên các cột phổ biến
        col_map = {}
...
def get_best_diem_chuan(series):
    # Lấy điểm chuẩn hợp lệ (khác N/A) nếu có nhiều dòng
    valid_scores = []
    for val in series:
        s = clean_score(val)
        if s != "N/A":
...
with st.container(border=True):
    ...
    def check_row_match(row_text, attr):
        ...
        if attr == "Trí tuệ nhân tạo (AI) trong kinh doanh":
            ...
            english_kws = ["tiên tiến", "ct tt", "tiếng anh toàn phần", "ct th", "úc", "new zealand", "mỹ", "hoa kỳ", "tiếng anh"]
        ...
        with st.spinner(t["analysis_searching"]):
            ...
            if df_dh is not None and not df_dh.empty:
                ...
                for col in df_dh.columns:
                    if "Tên ngành" in str(col):
                        nganh_col = col
                        break
                
                if nganh_col:
                    matching_rows = df_dh[df_dh[nganh_col].apply(lambda x: check_row_match(x, selected_attr))]
                    
                    if not matching_rows.empty:
                        # Gom nhóm theo Tên trường, Tên ngành, Mã ngành
                        required_cols = ["Tên trường", "Tên ngành", "Mã ngành"]
                        existing_req_cols = [c for c in required_cols if c in matching_rows.columns]
                        
                        grouped_data = []
                        if len(existing_req_cols) == 3:
                            groups = matching_rows.groupby(["Tên trường", "Tên ngành", "Mã ngành"], as_index=False)
                            stt = 1
                            for (truong, nganh, ma), group in groups:
                                diem_c = get_best_diem_chuan(group["Điểm chuẩn"]) if "Điểm chuẩn" in group.columns else "N/A"
                                ptxt_c = aggregate_ptxt(group["Phương thức xét tuyển"]) if "Phương thức xét tuyển" in group.columns else "N/A"
                                
                                grouped_data.append({
                                    "STT": stt,
                                    "Tên trường": clean_str(truong),
                                    "Tên ngành": clean_str(nganh),
                                    "Mã ngành": clean_str(ma),
                                    "Điểm chuẩn": diem_c,
                                    "Phương thức xét tuyển": ptxt_c
                                })
                                stt += 1
                            
                            df_result_final = pd.DataFrame(grouped_data)
                        else:
                            df_result_final = matching_rows.copy()
                            df_result_final.insert(0, 'STT', range(1, len(df_result_final) + 1))

                        st.markdown(f"### {t['analysis_table_title'].format(selected_attr)}")
                        st.markdown(render_custom_analysis_table(df_result_final), unsafe_allow_html=True)
                    else:
