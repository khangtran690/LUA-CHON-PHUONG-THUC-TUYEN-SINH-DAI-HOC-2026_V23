    ...
    if st.button(t["analysis_btn"]):
        with st.spinner(t["analysis_searching"]):
            ...
            if df_dh is None:
                ...
                df_dh = df_dh.loc[:, ~df_dh.columns.duplicated()]
                ...
                for _, row in df_dh.iterrows():
                    ...
                    if check_row_match(row_full_text, selected_attr):
                        matched_rows.append({
                            'Tên trường': clean_str(row.get('Tên trường', '')),
                            'Tên ngành': clean_str(row.get('Tên ngành', '')),
                            'Mã ngành': clean_str(row.get('Mã ngành', '')),
                            'Điểm chuẩn': clean_score(row.get('Điểm chuẩn', '')),
                            'Phương thức xét tuyển': clean_str(row.get('Phương thức xét tuyển', ''))
                        })
                ...
                if not matched_rows:
                    ...
                    table_html = render_custom_analysis_table(grouped_df)
